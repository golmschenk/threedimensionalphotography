...
class TestPointCloud:
    ...
    def test_finding_exact_transformation_to_another_point_cloud(self):
        ...
        transformation = point_cloud_r.attain_exact_transformation_to_point_cloud(point_cloud_l)

        assert np.allclose(transformation, expected_transformation)

    def test_finding_centroid(self):
        point_cloud = PointCloud(Point(1, 1, 1), Point(1, 3, 1), Point(3, 1, 1), Point(3, 3, 1))
        expected_centroid = Point(2, 2, 1)

        centroid = point_cloud.attain_centroid()
    ...
    def test_finding_transformation_to_another_point_cloud(self):
        ...
        point_cloud_l = PointCloud(Point(6, 4, 4.41421356),
                                   Point(6, 4.70710678, 9.36396103),
                                   Point(7, 1.17157288, 10.07106781),
                                   Point(9, 4, 4.41421356),
                                   Point(9, 4.70710678, 9.36396103),
                                   Point(9, 1.17157288, 10.07106781)
                                   )
        expected_transformation = np.array([[1, 0,        0,         5],
                                            [0, cos(π/4), -sin(π/4), 4],
                                            [0, sin(π/4),  cos(π/4), 3],
